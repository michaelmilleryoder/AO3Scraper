...
def write_fic_to_csv(fic_id, only_first_chap, writer, errorwriter, header_info=''):
    ...
    while True:
        ...
        except (ConnectionError, http.client.RemoteDisconnected, urllib.exceptions.ProtocolError):
            tqdm.write("Disconnected, attempting again in 1 min...")
            time.sleep(60)
            continue
        break
    src = req.text
    ...
    if (access_denied(soup)):
        ...
        author_name = soup.find('a', {'rel':'author'})
        if author_name is None:
            author_hash = 'Anonymous'
        else:
            author_hash = hashlib.md5(author_name.string.encode('utf8')).hexdigest()
        #get the fic itself
        #content = soup.find("div", id= "chapters")
        #chapters = content.select('p')
        #chaptertext = '\n\n'.join([unidecode(chapter.text) for chapter in chapters])
        #row = [fic_id] + [title] + [author_hash] + list(map(lambda x: ', '.join(x), tags)) + stats + [chaptertext]
        row = [fic_id] + [title] + [author_hash] + list(map(lambda x: ', '.join(x), tags)) + stats
        try:
            writer.writerow(row)
        except:
